...
muV=seq(-2,2,length.out=5)
sigmaV=seq(0.1,5, length.out=5)
dV=seq(5, 10, length.out=5)
rhoV=seq(-1, 1, length.out=5)
SV=c(10,100)
NV=c(10,100)

params=expand.grid(mu = muV, sigma = sigmaV,d = dV,rho=rhoV,S = SV, N = NV)
...
p=parallel::mclapply(1:nrow(params), mc.cores = nCores, FUN = function(i){
    ...
    X=makeX(mu,sigma,d,S,N)
    out<-optim(par=c(mu,sigma,d, rho),fn=.mc4opt,X=X,B=B, method = 'L-BFGS-B', 
               lower = c(-10, 0, 0,-1), upper = c(10, 10, 100,1),
               control = list(fnscale = -1))
    return(c(out$par, out$convergence))
})

p=do.call(rbind,p)
colnames(p) = c('mu_est', 'sigma_est', 'd_est', 'rho_est','convergence')
